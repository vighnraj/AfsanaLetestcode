...
const CounselorProfile = () => {
  const { id } = useParams(); // This 'id' is the correct one to use
 ...
 // In your useEffect where you fetch student data, format the date of birth
// In your useEffect where you fetch student data
useEffect(() => {
  if (id) {
    api
      ...
      .then((response) => {
        ...
        // Format the date of birth to YYYY-MM-DD format for the input field
        let formattedDateOfBirth = "";
        if (studentData.date_of_birth) {
          // Check if the date is a string and convert to Date object
          const dateObj = new Date(studentData.date_of_birth);
          // Format to YYYY-MM-DD (required format for input type="date")
        ...
        setApplicants(studentData.applicant_info || [initialApplicant]);
        setEPTs(studentData.english_proficiency_info || [initialEPT]);
        setJobs(studentData.job_professional_info || [initialJob]);
      })
      .catch((err) => {
...
const handleChange = (e) => {
  ...
  if (name === "date_of_birth" && value) {
    // The value from date input is already in YYYY-MM-DD format
...
};// Added id and navigate to dependency array
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Optionally, refetch student data to update the view
      api.get(`${BASE_URL}auth/getStudentById/${id}`).then(response => setStudent(response.data));
    } catch (err) {
  ...
  const handleUpload = async () => {
    // Use the 'id' from useParams, not localStorage
    if (!id) {
      alert("Student ID not found. Cannot upload documents.");
      return;
    }

    const data = new FormData();
    // Only append files that are actually selected
    if (formData.passport_copy_prepared) data.append("passport_copy_prepared", formData.passport_copy_prepared);
    if (formData.previous_studies_certificates) data.append("previous_studies_certificates", formData.previous_studies_certificates);
    if (formData.proof_of_income) data.append("proof_of_income", formData.proof_of_income);
    if (formData.birth_certificate) data.append("birth_certificate", formData.birth_certificate);
    if (formData.bank_statement) data.append("bank_statement", formData.bank_statement);

    try {
      // Use the 'api' instance for consistency, not 'axios' directly
      const res = await api.post(`${BASE_URL}postDocuments/${id}`, data, {
        headers: {
  ...
  return (
    <Container className="mt-4">
      ...
      <Modal show={showModal} onHide={() => setShowModal(false)} centered>
        ...
        <Modal.Body>
          <Form>
            ...
            <Form.Group className="mb-3">
              <Form.Label>Previous Studies Certificates</Form.Label>
      ...
      {/* Edit Profile Modal */}
      <Modal show={showProfileModal} onHide={handleCloseProfileModal} size="lg" scrollable>
        <Modal.Header closeButton>
