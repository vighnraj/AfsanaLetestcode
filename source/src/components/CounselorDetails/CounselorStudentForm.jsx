...
const CounselorStudentForm = () => {
  ...
  const { id } = useParams(); // Using react-router to get the student ID from URL
  ...
  useEffect(() => {
    // Fetch student data
    api
      .get(`${BASE_URL}auth/getStudentById/${id || studentId}`)
      .then((response) => {
  ...
  useEffect(() => {
    const fetchDocuments = async () => {
      ...
      try {
        ...
        const res = await axios.get(`${BASE_URL}getDocuments/${documentId}`);
  ...
  const InfoRow = ({ label, value }) =>
    value ? (
      <Row className="mb-2 align-items-center">
        ...
        <Col md={8}>{label === "Date of Birth:" ? formatDate(value) : value}</Col>
      </Row>
    ) : null;
  ...
  const handleDownload = (url, filename) => {
    ...
    // If the URL is not a full URL, prepend the BASE_URL
